...
private fun DrawScope.drawPrices(
    ...
    drawDashedLine(
        start = Offset(maxPriceOffsetY, maxPriceOffsetY),
    ...
    drawTextPrice(
        textMeasurer = textMeasurer,
        price = lastPriceOffsetY,
        offsetY = 0f
    )
    //  min
    val minPriceOffset = size.height
    drawDashedLine(
        start = Offset(0f, minPriceOffset - ((min - min) * pxPerPoint)),
        end = Offset(size.width, minPriceOffset),
    )
    drawTextPrice(
        textMeasurer = textMeasurer,
        price = min,
        offsetY = minPriceOffset
    )
}
